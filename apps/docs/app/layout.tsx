...
export const metadata = {
  title: 'Srcube Taro',
  description: 'A taro component.',
}

export default function RootLayout({ children }: PropsWithChildren) {
  const navItems = [
    { href: '/docs/guide/introduction', label: 'Docs' },
    { href: '/docs/components/button', label: 'Components' },
  ]

  return (
    <html lang="en" suppressHydrationWarning>
      <body className="mx-auto flex flex-col min-h-screen">
        <ThemeProvider attribute="class" defaultTheme="system" enableSystem>
          <header className="sticky top-0 z-40 w-full bg-white/95 dark:bg-black/95 backdrop-blur border-b border-gray-500/20">
            <div className="container flex gap-4 mx-auto max-w-screen-2xl h-16 items-center">
              <div className="mr-4 flex">
                <Link href="/" className="flex items-center space-x-4">
                  <Logo className="shrink-0 size-8" />
                  <span className="font-thin text-lg text-nowrap">SrcubeUI Taro</span>
                </Link>
              </div>
          ...
          <main className="flex-1 mx-auto max-w-screen-2xl w-full">
            {children}
          </main>
        </ThemeProvider>
      </body>
